...
public class SwiftLoader: UIView {
    ...
    public class func show(title: String?, animated : Bool) {
        ...
        guard let currentWindow : UIWindow = UIApplication.shared.windows.filter({$0.isKeyWindow}).first
        else {
            print("‼️‼️‼️ SwiftLoader: Can't find active Window !!!")
            return
        }
