        ...
        VStack (alignment: .leading){
            ...
            if fromSearch {
                ...
                if let reels = reels {
                    ...
                    let a  = Array(reels.unflattening(dim: 3).prefix(2))
                    let b  = Array(reels.unflattening(dim: 3).prefix(1))
                    if a.count > 0  {
                        VStack (alignment: .leading, spacing: 12){
                            ForEach(0...a.count - 1, id: \.self) { index in
                                let thisReel = a[index]
                                HStack (spacing: 12){
                        ...
                        VStack (alignment: .leading, spacing: 12){
                            ForEach(0...b.count - 1, id: \.self) { index in
                                let thisReel = b[index]
                                HStack (spacing: 12 ){
                                    ForEach(thisReel, id: \.id) { reel in
                                        ReelsView(reel: reel) { reel in
                                            reelTapped(reel: reel)
                ...
                if let reels = reels {
                    ...
                    let reelsFlattening = Array(reels.unflattening(dim: 3).prefix(2))
                    let reelsFlatte = Array(reels.unflattening(dim: 3).prefix(2))
                    
                    if reelsFlattening.count > 0 {
                        VStack (spacing: 12){
                    ...
                    } else if reelsFlatte.count > 0 {
                        VStack (spacing: 12){
                            ForEach(0...reelsFlatte.count - 1, id: \.self) { index in
                                let thisReel = reelsFlatte[index]
                                HStack (spacing: 12){
                                    ForEach(thisReel, id: \.id) { reel in
                                        ReelsView(reel: reel) { reel in
                                            reelTapped(reel: reel)
