...
// MARK: - ReelsCacheManagerDelegate
...
class ReelsCacheManager {
    ...
    func begin(reelModel: Reel, position: Int) {
        ...
        queue.async { [weak self] in
            self?.fetchResFileList(reelModel, position)
        }
        ...
        let task = URLSession.shared.dataTask(with: url) { [weak self] data, _, _ in
            guard let data = data else {
                DispatchQueue.global().asyncAfter(deadline: .now() + 2) {
                    self?.fetchResFileList(reel, position)
                }
                return
            }
    ...
    func fetchFileChunks(_ urlString: String, _ reel: Reel, _ position: Int) {
        let url = URL(string: urlString)!
        let task = URLSession.shared.dataTask(with: url) { [weak self] data, _, _ in
            guard let data = data else {
                DispatchQueue.global().asyncAfter(deadline: .now() + 2) {
                    self?.fetchFileChunks(urlString, reel, position)
                }
                return
            }
    ...
    func downloadChunkAndStore(_ chunkContent: String, _ reel: Reel, _ position: Int) {
        ...
        if urlChunck != " ", urlChunck.count > 0 {
            ...
            let task = URLSession.shared.dataTask(with: url) { data, _, _ in
                guard let data = data else {
                    DispatchQueue.global().asyncAfter(deadline: .now() + 2) {
                        self.downloadChunkAndStore(chunkContent, reel, position)
                    }
                    return
                }
    ...
    private func completion(reel: Reel, position: Int, finalM3U8FileName: String) {
        if let webserver = webServerHost {
            self.reel = reel
            self.reel.media = webserver + "M3U8Files/" + "\(position)_" + (reel.id ?? "") + "_" + finalM3U8FileName
            delegate?.cachingCompleted(reel: self.reel, position: position)
        }
    }
...
//            }
    ...
    func createDirIfNeeded(dirName: String) {
        ...
        do {
            ...
            DispatchQueue.global().asyncAfter(deadline: .now() + 2) {
                self.createDirIfNeeded(dirName: dirName)
            }
    ...
    func deleteCachedReels(currentPosition: Int, reelsArray: [Reel], completion: @escaping (_ index: Int) -> Void) {
        let fileManager = FileManager.default
        ...
        do {
            ...
            DispatchQueue.global().asyncAfter(deadline: .now() + 2) {
                self.deleteCachedReels(currentPosition: currentPosition, reelsArray: reelsArray, completion: completion)
            }
        }
    }
}
