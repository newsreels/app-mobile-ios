...
open class EZPlayer: NSObject {
    ...
    private var customSeekTime: CMTime?
    ...
    /// 上下滑动屏幕的控制类型
    open var slideTrigger = (left:EZPlayerSlideTrigger.volume,right:EZPlayerSlideTrigger.volume)//(left:EZPlayerSlideTrigger.volume,right:EZPlayerSlideTrigger.brightness)
    /// 左右滑动屏幕改变视频进度
...
//                    SharedManager.shared.showAlertLoader(message: "Some error occured. Video playing failed.", type: .alert)
    ...
    // MARK: - Player action
    open func playWithURL(_ url: URL,embeddedContentView contentView: UIView? = nil, title: String? = nil, playTime: CMTime? = nil) {

        self.contentItem = EZPlayerContentItem(url: url, title: title)
        self.contentURL = url

        self.customSeekTime = playTime
            ...
            if item == self.playerItem {
                ...
                case #keyPath(AVPlayerItem.status):
                    ...
                    if item.status == .readyToPlay {//可以播放
                        ...
                        if self.customSeekTime != nil {
                            self.player?.seek(to: self.customSeekTime!, completionHandler: { status in
                                print("status",status)
                            })
