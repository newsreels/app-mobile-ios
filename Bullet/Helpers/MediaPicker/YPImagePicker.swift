...
open class YPImagePicker: UINavigationController {
    ...
    let loadingView = UIActivityIndicatorView()//YPLoadingView()
...
//                    self?.didSelect(items: [mediaItem])
                    ...
                    let photoCropVC = CropperVC.instantiate(fromAppStoryboard: .Home)
                    photoCropVC.selectedImage  = photo.originalImage
                    photoCropVC.didSave = { [weak self] outputMedia in
                        self?.didSelect(items: [outputMedia])
                    }
                    self?.pushViewController(photoCropVC, animated: true)
...
//                                                                   isFromSelectionVC: false)
                    if YPConfig.isForReels {
                        
                        let videoCropVC = ReelsCropperViewController.instantiate(fromAppStoryboard: .Home)
                        videoCropVC.inputVideo  = video
                        videoCropVC.didSave = { [weak self] outputMedia in
                            self?.didSelect(items: [outputMedia])
                        }
                        self?.pushViewController(videoCropVC, animated: true)
