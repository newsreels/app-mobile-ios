...
public class YPLibraryVC: UIViewController, YPPermissionCheckable {
    ...
    func initialize() {
        ...
        case .all:
            ...
            v.maxNumberWarningLabel.text = String(format: NSLocalizedString("Warning Max Items Limit", comment: ""),
        ...
        case .photo:
            v.assetViewContainer.multipleSelectionButton.isHidden = !(YPConfig.libraryPhotoOnly.maxNumberOfItems > 1)
            v.maxNumberWarningLabel.text = String(format: NSLocalizedString("Warning Max Items Limit", comment: ""),
                                                  YPConfig.libraryPhotoOnly.maxNumberOfItems)
            
            if let preselectedItems = YPConfig.libraryPhotoOnly.preselectedItems, !preselectedItems.isEmpty {
                selection = preselectedItems.compactMap { item -> YPLibrarySelection? in
                    var itemAsset: PHAsset?
                    switch item {
                    case .photo(let photo):
            ...
            if YPConfig.libraryPhotoOnly.defaultMultipleSelection || selection.count > 1 {
                showMultipleSelection()
            }
            
        case .video:
            v.assetViewContainer.multipleSelectionButton.isHidden = !(YPConfig.libraryVideoOnly.maxNumberOfItems > 1)
            v.maxNumberWarningLabel.text = String(format: NSLocalizedString("Warning Max Items Limit", comment: ""),
                                                  YPConfig.libraryVideoOnly.maxNumberOfItems)
            
            if let preselectedItems = YPConfig.libraryVideoOnly.preselectedItems, !preselectedItems.isEmpty {
                selection = preselectedItems.compactMap { item -> YPLibrarySelection? in
                    var itemAsset: PHAsset?
                    switch item {
                    case .photo(let photo):
            ...
            if YPConfig.libraryVideoOnly.defaultMultipleSelection || selection.count > 1 {
                showMultipleSelection()
            }
    ...
    public override func viewDidAppear(_ animated: Bool) {
        ...
        case .photo:
            if !YPConfig.libraryPhotoOnly.onlySquare && v.assetZoomableView.contentSize == CGSize(width: 0, height: 0) {
                v.assetZoomableView.setZoomScale(1, animated: false)
            }
            
            // Activate multiple selection when using `minNumberOfItems`
            if YPConfig.libraryPhotoOnly.minNumberOfItems > 1 {
                multipleSelectionButtonTapped()
            }
        case .video:
            if !YPConfig.libraryVideoOnly.onlySquare && v.assetZoomableView.contentSize == CGSize(width: 0, height: 0) {
                v.assetZoomableView.setZoomScale(1, animated: false)
            }
            
            // Activate multiple selection when using `minNumberOfItems`
            if YPConfig.libraryVideoOnly.minNumberOfItems > 1 {
                multipleSelectionButtonTapped()
            }
        default:
            if !YPConfig.library.onlySquare && v.assetZoomableView.contentSize == CGSize(width: 0, height: 0) {
    ...
    func showMultipleSelection() {
        ...
        case .photo:
            if YPConfig.libraryPhotoOnly.minNumberOfItems > 1 && multipleSelectionEnabled {
                return
            }
            ...
            if multipleSelectionEnabled {
                if selection.isEmpty && YPConfig.libraryPhotoOnly.preSelectItemOnMultipleSelection,
                    delegate?.libraryViewShouldAddToSelection(indexPath: IndexPath(row: currentlySelectedIndex, section: 0),
                                                              numSelections: selection.count) ?? true {
                    let asset = mediaManager.fetchResult[currentlySelectedIndex]
                    selection = [
        ...
        case .video:
            if YPConfig.libraryVideoOnly.minNumberOfItems > 1 && multipleSelectionEnabled {
                return
            }
            ...
            if multipleSelectionEnabled {
                if selection.isEmpty && YPConfig.libraryVideoOnly.preSelectItemOnMultipleSelection,
                    delegate?.libraryViewShouldAddToSelection(indexPath: IndexPath(row: currentlySelectedIndex, section: 0),
                                                              numSelections: selection.count) ?? true {
                    let asset = mediaManager.fetchResult[currentlySelectedIndex]
                    selection = [
    ...
    func refreshMediaRequest() {
        ...
        if mediaManager.hasResultItems {
            ...
            case .photo:
                if !multipleSelectionEnabled && YPConfig.libraryPhotoOnly.preSelectItemOnMultipleSelection {
                    addToSelection(indexPath: IndexPath(row: 0, section: 0))
                }
            case .video:
                if !multipleSelectionEnabled && YPConfig.libraryVideoOnly.preSelectItemOnMultipleSelection {
                    addToSelection(indexPath: IndexPath(row: 0, section: 0))
                }
            default:
                if !multipleSelectionEnabled && YPConfig.library.preSelectItemOnMultipleSelection {
            ...
            let popup = YPPermissionDeniedPopup()
            let alert = popup.popupNoPhotos(cancelBlock: {
    ...
    func buildPHFetchOptions() -> PHFetchOptions {
        ...
        case .photo:
            if let userOpt = YPConfig.libraryPhotoOnly.options {
                return userOpt
            }
            options.predicate = YPConfig.libraryPhotoOnly.mediaType.predicate()
        case .video:
            if let userOpt = YPConfig.libraryVideoOnly.options {
                return userOpt
            }
    ...
    private func fetchVideoAndApplySettings(for asset: PHAsset,
        ...
        guard fitsVideoLengthLimits(asset: asset) else {
            delegate?.libraryViewDidTapNextFailedWithError()
            return
        }
...
//        }
        ...
        saveTempCopy(for: asset, callback: callback)
    }
...
//        mediaManager.fetchVideoUrlAndCropWithDuration(for: asset, cropRect: rect, duration: timeDuration, callback: callback)
    }
    
    
    func saveTempCopy(for videoAsset: PHAsset,callback: @escaping (_ videoURL: URL?) -> Void) {
        
        PHImageManager.default().requestAVAsset(forVideo: videoAsset, options: PHVideoRequestOptions(), resultHandler: { (asset, audioMix, info) -> Void in
            if let asset = asset as? AVURLAsset {
//                let videoData = Data(contentsOfURL: asset.URL)
                
                // optionally, write the video to the temp directory
                let videoPath = NSTemporaryDirectory() + asset.url.lastPathComponent
                let videoURL = URL(fileURLWithPath: videoPath)
                
                let fileCopyState = FileManager.default.secureCopyItem(at: asset.url, to: videoURL)
                if fileCopyState {
                    print("file copy successfull", videoURL)
                    callback(videoURL)
                    self.delegate?.libraryViewDidTapNext()
                } else {
                    callback(nil)
                    self.delegate?.libraryViewDidTapNextFailedWithError()
                }
                        ...
                        self.fetchVideoAndApplySettings(for: asset.asset,
                            ...
                            if let videoURL = videoURL {
                                let videoItem = YPMediaVideo(thumbnail: thumbnailFromVideoPath(videoURL),
                                                             videoURL: videoURL, asset: asset.asset, taskID: "")
                        ...
                        DispatchQueue.main.async {
                            if let videoURL = videoURL {
                                ...
                                let video = YPMediaVideo(thumbnail: thumbnailFromVideoPath(videoURL),
                                                         videoURL: videoURL, asset: asset, taskID: "")
                                videoCallback(video)
                            } else {
                                print("YPLibraryVC -> selectedMedia -> Problems with fetching videoURL.")
                            }
