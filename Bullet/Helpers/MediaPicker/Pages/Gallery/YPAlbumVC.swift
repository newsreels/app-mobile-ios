...
class YPAlbumVC: UIViewController {
    ...
    var selectionType: YPLibraryVC.mediaType?
    
    required init(albumsManager: YPAlbumsManager, selectedType: YPLibraryVC.mediaType) {
    ...
    func fetchAlbumsInBackground() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            self?.albums = self?.albumsManager.fetchAlbums(selectionType: self?.selectionType ?? .all) ?? []
