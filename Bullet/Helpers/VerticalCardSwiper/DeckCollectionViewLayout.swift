...
@IBDesignable open class DeckCollectionViewLayout: UICollectionViewLayout {
    ...
    @IBInspectable open var cardCount = 1 {
        didSet { invalidateLayout() }
    }

    /// Insets of the deck inside the content area.
    @IBInspectable open var deckInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0) {
        didSet { invalidateLayout() }
    }
    ...
    @IBInspectable open var cardOffsetDelta = CGPoint(x: 0, y: 0) {
        didSet { invalidateLayout() }
    }

    open override func prepare() {
        contentRect = collectionView!.bounds.inset(by: collectionView!.contentInset)
        cardFrame = contentRect.inset(by: deckInsets)
        cardScaleFactor = min(cardFrame.width.scaleFactor(delta: cardSizeDelta.width), cardFrame.height.scaleFactor(delta: cardSizeDelta.height))
