...
class AddTagVC: UIViewController {
    ...
    func setLocalizable() {
        ...
        lblTitle.text = NSLocalizedString("Add Tag", comment: "")
        lblTagTitle.text = NSLocalizedString("Add topics tags", comment: "")
        lblTagDesc.text = NSLocalizedString("Be discovered by people following the topics you are posting about.", comment: "")
...
extension AddTagVC: UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if !(text.isEmpty) && string == "," {
            ...
            if let firstSuchElement = self.arrTags?.first(where: { $0.name == text }) {
                ...
                SharedManager.shared.showAlertLoader(message: NSLocalizedString("Tag Already Added.", comment: ""))
            }
            else {
                tagListView.addTag(text)
                self.arrTags?.append(Tags(id: nil, name: text, icon: nil, image: nil))
                textField.text = ""
            }
            return false
        }
...
extension AddTagVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        tagCell.imgTag?.sd_setImage(with: URL(string: tag?.icon ?? ""), placeholderImage: nil)
    ...
    func selectedTagAddOrRemove(isAdd: Bool, tag: Tags) {
        ...
        if isAdd {
            ...
            self .performWSToAddTags(articleId: articleID, tagName: name)
        }
        else {
...
extension AddTagVC {
    ...
    func performWSToGetSuggestedTagsList(articleId: String, searchText: String) {
        ...
        let apiUrl : String = "studio/\(articleId)/tags/suggestion?query=\(searchText)"
    ...
    func performWSToAddTags(articleId: String, tagName: String) {
        ...
        WebService.URLResponse(apiUrl, method: .post, parameters: params, headers: token, withSuccess: { (response) in
            ...
            do{
                ...
                if let arrTags = FULLResponse.tags {
