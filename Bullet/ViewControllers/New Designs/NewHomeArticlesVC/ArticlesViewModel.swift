...
class ArticlesViewModel: NSObject {
    ...
    var callbackGetCategories: ((_ response: Data?, _ error: ApiErrorType?) -> ())?
    var callbackRefreshCategories: ((_ response: Data?, _ error: ApiErrorType?) -> ())?
    ...
    func performWSToGetHomeCategories() {
        
        let params = ["reader_mode": SharedManager.shared.readerMode]

        
        if !(SharedManager.shared.isConnectedToNetwork()){
            self.callbackGetCategories?(nil, .internetError)
            return
        }
        
        
        let token = UserDefaults.standard.string(forKey: Constant.UD_userToken)
        WebService.URLResponse("news/home?type=articles", method: .get, parameters: params, headers: token, withSuccess: { (response) in

            self.callbackGetCategories?(response, nil)
            
        }) { (error) in
            
            self.callbackGetCategories?(nil, .jsonError)
        }
        
    }
    
    func performWSToRefreshHomeCategories() {
        
        let params = ["reader_mode": SharedManager.shared.readerMode]

        
        if !(SharedManager.shared.isConnectedToNetwork()){
            self.callbackRefreshCategories?(nil, .internetError)
            return
        }
        ...
        WebService.URLResponse("news/home?type=articles", method: .get, parameters: params, headers: token, withSuccess: { (response) in
            ...
            self.callbackRefreshCategories?(response, nil)
            
        }) { (error) in
            
            self.callbackRefreshCategories?(nil, .jsonError)
        }
        
    }
    
    
    func performWSToGetTopicDetailsWithReels(isReloadView: Bool = false, newPost: Bool = false, id: String, nextPaginate: String) {
        
        let params = ["context": id,
                      "reader_mode": SharedManager.shared.readerMode,
                      "page": nextPaginate,
                      "reels": true] as [String : Any]
