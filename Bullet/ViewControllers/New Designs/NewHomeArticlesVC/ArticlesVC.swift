...
protocol ArticlesVCScrollDelegate: AnyObject {
    func homeScrollViewDidScroll(delta: CGFloat)
}

protocol ArticlesVCDelegate: AnyObject {
...
class ArticlesVC: UIViewController, UIGestureRecognizerDelegate {
...
//    @IBOutlet weak var extendedViewBottomConstraint: NSLayoutConstraint!
    ...
    @IBOutlet weak var loaderTopConstraint: NSLayoutConstraint!
    
    @IBOutlet weak var articleMenuIcon: UIImageView!
    @IBOutlet weak var topicFollowIcon: UIImageView!
    @IBOutlet var backButotn: UIButton!
    var timer: Timer?
    
    var gradientLayer: CAGradientLayer?
    let forYouCategoryID = "VE9QX05FV1MqKioqKkZPUl9ZT1U="
    
    var currentSelectedCategoryID = ""
    ...
    var normalTableViewTopConstraint: CGFloat = 86
...
//    var layouted: Bool = false
    ...
    weak var scrollDelegate: ArticlesVCScrollDelegate?
...
//    }
    ...
    // View Model
    var homeViewModel : ArticlesViewModel!
    
    var currentPageIndex = 0
    var isOnFollowing = false
    weak var delegate: ArticlesVCDelegate?
    var placeContextId = ""
    var topicContextId = ""
    var subTopicTitle = ""
    var selectedID = ""
    ...
    var isOpenFromAricleTopic : Bool = false
    var arrFilteredSections = [sectionsData]()
...
//        }
        ...
        getAllData(isreloadDataNeeded: true)
        
        refreshCategoryList()
        NotificationCenter.default.addObserver(self, selector: #selector(tabBarTapped(notification:)), name: Notification.Name.notifyArticlesTabBarTapped, object: nil)

        
        if fromDiscover || isOpenFromAricleTopic{
            self.loadTopicsFromDiscover()
        }
...
//        print("OFFSET = \(self.tblExtendedView.contentOffset)")
...
//
////            pullToRefreshAction()
////            self.tblExtendedView.
//            let indexPath = IndexPath(item: 0, section: 0)
//            self.tblExtendedView.scrollToRow(at: indexPath, at: .top, animated: true)
//        } else {
//
...
//        }
        ...
        if SharedManager.shared.isTabReload {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) {
                self.arrFilteredSections.removeAll()
                self.getRefreshArticlesData(startFromFirstPosition: true)
            }
        }
    }
...
//        showLoader()
        ...
        self.backButotn.isHidden = !fromDiscover
        self.backWidthConst.constant = fromDiscover ? 44 : 20
//        self.menuView.isHidden = fromDiscover
        self.dateLabel.isHidden = fromDiscover
        
        NotificationCenter.default.addObserver(self, selector: #selector(loadNewData), name: NSNotification.Name(rawValue: "OpenedFromBackground"), object: nil)

        if appDelegate.shouldResetArticles {
            loadNewData()
            appDelegate.shouldResetArticles = false
        }
    }
...
//                self.getRefreshArticlesData(startFromFirstPosition: true)
            ...
            getAllData(isreloadDataNeeded: false)
        }
...
//        NotificationCenter.default.removeObserver(SharedManager.shared.observerArray)
    ...
    func getAllData(isreloadDataNeeded: Bool) {
        
        if SharedManager.shared.articlesCategories.count == 0 || getCategoryId() == "" {
            self.loadCategories()
        }
//        else if let arrCache = self.readCache(key: getCategoryId()), arrCache.count > 0 {
//            if isreloadDataNeeded {
//                self.getRefreshArticlesData(startFromFirstPosition: true)
//            }
//        }
        else {
    ...
    func setupNavView() {
        ...
        if SharedManager.shared.curArticlesCategoryId == forYouCategoryID {
            
            if fromDiscover {
                greetingLabel.text = categoryTitleString
            } else {
                greetingLabel.text = SharedManager.shared.getGreeting()
            }
            
            dateLabel.text = SharedManager.shared.getTodaysDay()
            greetingLabel.textColor = UIColor.black
            
            normalTableViewTopConstraint = 86
            tableViewTopConstraint.constant = normalTableViewTopConstraint
            
            startGreetingTimer()
        }
        else {
            if let cat = SharedManager.shared.articlesCategories.first(where: { $0.id == SharedManager.shared.curArticlesCategoryId }) {
                
                greetingLabel.text = cat.title ?? ""
                greetingLabel.textColor = Constant.appColor.lightRed
                
                normalTableViewTopConstraint = 56
            }
            else {
        ...
        let grad = SharedManager.shared.getGradient(viewGradient: navView, colours: [UIColor.white, Constant.appColor.backgroundGray], locations: nil, startPoint: CGPoint(x: 0.8, y: 0.8), endPoint: CGPoint(x: 0.8, y: 1.0))
        ...
        if fromDiscover {
            ...
            articleMenuIcon.isHidden = false
            topicFollowIcon.isHidden = true
            greetingLabel.textColor = Constant.appColor.lightRed
        }
    ...
    func startGreetingTimer() {
        ...
        timer = Timer(timeInterval: 1, repeats: true, block: { timer in
            ...
            if SharedManager.shared.curArticlesCategoryId == self.forYouCategoryID {
                
                if self.fromDiscover {
                    self.greetingLabel.text = self.categoryTitleString
                } else {
                    self.greetingLabel.text = SharedManager.shared.getGreeting()
                }
                self.dateLabel.text = SharedManager.shared.getTodaysDay()
            }
        })
        timer?.fire()
    }
    
    func stopGreetingTimer() {
    ...
    func showCircularLoader(isLoadingCategory: Bool) {
        
        if isLoadingCategory {
            navView.alpha = 0
            loaderTopConstraint.constant = 0
        }
        else {
//            if navView.alpha == 0 {
...
//                }
//            }
//            loaderTopConstraint.constant = normalTableViewTopConstraint + view.safeAreaLayoutGuide.layoutFrame.origin.y
        }
        
        self.loaderView.isHidden = false
    }
    
    func hideCircularLoader() {
    ...
    func loadTopicsFromDiscover() {
        
        viewNewPosts.isHidden = true
        DataCache.instance.clean(byKey: contextID ?? "")
    ...
    // Chack refresh needed if we already have category list
    func refreshCategoryList() {
        
        if SharedManager.shared.articlesCategories.count > 0 {
//            self.homeViewModel.performWSToRefreshHomeCategories()
        }
...
//        lblNoDataTitle.text = NSLocalizedString("There is nothing in here", comment: "")
        lblNoDataTitle.text = NSLocalizedString("Sorry! no result have been found :(", comment: "")
...
//        lblForcingGetStart.text = NSLocalizedString("LET'S GO", comment: "")
    ...
    func setDesignView() {
        ...
        loaderView.backgroundColor = Constant.appColor.backgroundGray
        setupNavView()
        
        viewNewPosts.isHidden = true
        //        view.theme_backgroundColor = GlobalPicker.customTabbarBGColor
//        view.theme_backgroundColor = GlobalPicker.bgBlackWhiteColor
        self.view.backgroundColor = Constant.appColor.backgroundGray
...
//            self.tblExtendedView.showSkeleton()
            ...
            self.showCircularLoader(isLoadingCategory: false)
            self.tblExtendedView.reloadData()
        }
//        DispatchQueue.main.async {
//
...
//        }
    ...
    func pullToRefreshAction(isBackground: Bool = false) {
        ...
        self.arrFilteredSections.removeAll()
        self.cacheOffset = 10
        self.currentImgCachePosition = 0
        ArticleCacheManager.shared.clearDiskCache()
...
//        SharedManager.shared.curCategoryId = SharedManager.shared.headlinesList.first?.id ?? ""
    ...
    @IBAction func didTapMenu(_ sender: Any) {
        ...
        if fromDiscover, let topic = topicData {
            topicFollowIcon.image = UIImage()
            topicFollowIcon.showLoader(size: CGSize(width: 15, height: 15),color: Constant.appColor.lightRed, padding: 0)
            SharedManager.shared.performWSToUpdateUserFollow(id: [topic.id ?? ""], isFav: !(topic.favorite!), type: .topics) { status in
                self.topicFollowIcon.hideLoaderView()
                if status {
                    self.topicData.favorite = !(topic.favorite!)
                    self.topicFollowIcon.image = !(topic.favorite!) ? UIImage(named: "tickSelected") : UIImage(named: "plus_dark")
                }
            }
            return
        }
        
        SharedManager.shared.filterType = "articles"

        isViewPresenting = false
        updateProgressbarStatus(isPause: true)
        
        currentSelectedCategoryID = SharedManager.shared.curArticlesCategoryId
        
        let vc = ForYouPreferencesVC.instantiate(fromAppStoryboard: .Reels)
        vc.preferenceType = .articles
        vc.delegate = self
        vc.currentCategory = SharedManager.shared.curArticlesCategoryId
...
extension ArticlesVC {
    ...
    func loadCategories() {
        ...
        let setUpHomeCategory = {
            ...
            self.showCircularLoader(isLoadingCategory: true)
            self.homeViewModel.performWSToGetHomeCategories()
        ...
        if SharedManager.shared.isTabReload {
            ...
            //Read from cache
            do {
                let categories: [MainCategoriesData]? = try DataCache.instance.readCodable(forKey: Constant.CACHE_ARTICLES_CATEGORIES)

                if let categories = categories, categories.count > 0 {
                    SharedManager.shared.articlesCategories = categories
    ...
    func getRefreshArticlesData(startFromFirstPosition: Bool = false) {
        
        // After loading categories load news
        // News loading 
        ...
        if self.showArticleType != .topic && self.showArticleType != .places {
            ...
            if nextPaginate.isEmpty && !isPullToRefresh {
                ...
                let id = fromDiscover ? (contextID ?? self.getCategoryId()) : self.getCategoryId()
...
//                        })
    ...
    func getCategoryId(isReloadView: Bool = false) -> String {
        ...
        var id = ""
        if self.showArticleType == .topic || fromDiscover{
            
            id = fromDiscover ? (contextID ?? topicContextId) : topicContextId
            ...
            if SharedManager.shared.articlesCategories.count > 0 {
                if SharedManager.shared.articlesCategories.contains(where: { $0.id == SharedManager.shared.curArticlesCategoryId }) {
                    
                        id = SharedManager.shared.curArticlesCategoryId
...
//            print("home id: ", nextPaginate, SharedManager.shared.headlinesList[self.pageIndex].title ?? "")
        }
...
//        }
    ...
    func removeDuplicateElementsForCategory(feeds: [sectionsData]) -> [sectionsData] {
         var arrFeedFiltered = [sectionsData]()
         for feed in feeds {
             if !arrFeedFiltered.contains(where: {$0.type == feed.type }) {
                 arrFeedFiltered.append(feed)
             }
         }
    ...
    func loadDataInFeedList(_ arrFeed: [sectionsData], id: String, isNewPost: Bool = false) {
        ...
        let arrFeedFiltered = SharedManager.shared.curArticlesCategoryId == self.forYouCategoryID ? removeDuplicateElements(feeds: arrFeed) : removeDuplicateElementsForCategory(feeds: arrFeed)
//        if arrFeedFiltered.count == 0 && articles.count == 0 {
//            arrFilteredSections.removeAll()
        ...
        print(arrFeedFiltered)
            for (_, dictData) in arrFeedFiltered.enumerated() {
                ...
                if feedType == Constant.newsArticle.FEED_ARTICLE_HERO && feedType != "" {
                    ...
                    print("Banner header = \(bannerHeader)")
                    if let articles = feedData?.articles {
                        
                        //Header
                        if bannerHeader != "" {
                            if !arrArticles.contains(where: {$0.title == bannerHeader}) {
                                arrArticles.append(articlesData(id: feedData?.context, title: bannerHeader, subheader: feedData?.subheader ?? "", type: Constant.newsArticle.ARTICLE_TYPE_HEADER, footer: feedData?.footer))
                                SharedManager.shared.performWSToUpdateArticleAnalytics(ArticleId: feedData?.context ?? "", isFromReel: false)
                            }
                        }
...
extension ArticlesVC: BottomSheetVCDelegate {
...
extension ArticlesVC: HomeCardCellDelegate, YoutubeCardCellDelegate, VideoPlayerViewwDelegates, FullScreenVideoVCDelegate {
...
//            vCell.resetVisibleVideoPlayer()
    ...
    func layoutUpdate() {
        ...
        DispatchQueue.main.async {
            if self.prefetchState != .fetching && self.articles.count == self.tblExtendedView.numberOfRows(inSection: 0) {
                self.tblExtendedView.beginUpdates()
                self.tblExtendedView.endUpdates()
            }
...
//        self.presentDetail(navVC)
    ...
    @objc func didTapForYouHeaderAction(button: UIButton) {
        ...
        SharedManager.shared.curArticlesCategoryId = content.id ?? ""
        NotificationCenter.default.post(name: Notification.Name.notifyTapSubcategories, object: nil)
    }
...
extension ArticlesVC: UIActivityItemSource {
...
//                headerCell.backgrView.theme_backgroundColor = GlobalPicker.backgroundColorWhiteBlack
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let headerCell = tableView.dequeueReusableCell(withIdentifier: HEADER_HOME_CC, for: indexPath) as? HomeHeaderCC {
//            if let  title = self.articles[indexPath.row].title{
//                let vc = ArticlesVC.instantiate(fromAppStoryboard: .Main)
//                vc.fromDiscover = false
...
extension ArticlesVC: UIScrollViewDelegate, UICollectionViewDelegate {
        ...
        if self.tableViewTopConstraint.constant != self.extendedTableViewTopConstraint {
            ...
            if animated {
                UIView.animate(withDuration: 0.25) {
                    ...
                    self.navTopConstraint.constant = -(self.navView.frame.size.height)
                ...
                self.tableViewTopConstraint.constant = self.extendedTableViewTopConstraint
                self.navTopConstraint.constant = -(navView.frame.size.height)
                self.navView.alpha = 0
                self.view.layoutIfNeeded()
            }
            self.delegate?.homeScrollViewDidScroll(delta: 1, animated: animated)
...
extension ArticlesVC: LikeCommentDelegate {
...
extension ArticlesVC: BulletDetailsVCLikeDelegate {
...
extension ArticlesVC: CommentsVCDelegate {
...
// MARK: - Ads
// Google Ads
extension ArticlesVC: GADUnifiedNativeAdLoaderDelegate {
...
// Facebook Ads
extension ArticlesVC: FBNativeAdDelegate {
...
extension ArticlesVC: sugClvReelsCCDelegate, HomeClvHeadlineCCDelegate, HomeHeroCCDelegate, sugClvTopicsCCDelegate {
...
extension ArticlesVC: AuthorProfileVCDelegate {
...
//MARK:- Popup Delegate
extension ArticlesVC: PopupVCDelegate {
...
extension ArticlesVC: CommunityGuideVCDelegate {
...
extension ArticlesVC: YoutubeArticleVCDelegate {
...
extension ArticlesVC: YPImagePickerDelegate {
...
extension ArticlesVC: SharingDelegate, UIDocumentInteractionControllerDelegate {
...
extension ArticlesVC: HomeReelCarouselCCDelegate, HomeVideoCarouselCCDelegate, ReelsVCDelegate, BulletDetailsVCDelegate {
...
extension ArticlesVC: SuggestedAuthorsCCDelegate {
...
extension ArticlesVC: ForYouPreferencesVCDelegate {
...
//        getRefreshArticlesData(startFromFirstPosition: true)
    }
    ...
    func userDismissed(vc: ForYouPreferencesVC, selectedPreference: Int, selectedCategory: String) {
        ...
        if SharedManager.shared.curArticlesCategoryId != currentSelectedCategoryID {

            isViewPresenting = true
//            getRefreshArticlesData(startFromFirstPosition: true)
            isOpenFromAricleTopic = true
            loadNewData()
        }
        else {
...
//
extension ArticlesVC: ArticleCacheManagerDelegate {
    func cachingCompleted(article: articlesData, position: Int) {

        if position < self.articles.count {
            self.articles[position] = article
//            DispatchQueue.main.async {
...
//            }
        ...
        currentImgCachePosition += 1
        startArticleCaching()
    }



    func startArticleCaching() {
        ArticleCacheManager.shared.delegate = self

        if currentImgCachePosition < self.articles.count && currentImgCachePosition < cacheOffset {
            if self.articles[currentImgCachePosition].type == "HEADER"{
                currentImgCachePosition += 1
                ArticleCacheManager.shared.downloadChunkAndStore(self.articles[currentImgCachePosition], currentImgCachePosition)
                return
            }
            ArticleCacheManager.shared.downloadChunkAndStore(self.articles[currentImgCachePosition], currentImgCachePosition)
        }
