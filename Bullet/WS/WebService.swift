...
//    class func getAccessToken( parameters : [String: String], completion: @escaping (Result<Data, Error>) -> Void) {
...
//        ]
//        
//        // Perform POST request with Alamofire
//        AF.request(url, method: .post, parameters: parameters, encoding: URLEncoding.default, headers: headers)
//            .validate() // Ensures a valid response status code
//            .responseData { response in
//                switch response.result {
//                case .success(let data):
//                    // Return bytecode data if successful
...
//    parameters["id_token"] = subjectToken
...
//        }
//    }
    class func callApi(url : String, parameters : [String: String], completion: @escaping (Result<Data, Error>) -> Void) {
        // Parameters for the request
        // Headers for form-urlencoded content type
        let headers: HTTPHeaders = [
            "Content-Type": "application/x-www-form-urlencoded"
        ...
        AF.request(completeUrl, method: method, parameters: parameters, encoding: URLEncoding.default, headers: headersToken).validate(statusCode: 200 ..< 600).responseData(completionHandler: { response in
            ...
            case let .success(value):
                ...
                let responseString = String(data: value, encoding: .utf8)
                 print("TermsVs Response String: \(responseString)")
                let appDelegate = UIApplication.shared.delegate as! AppDelegate
                if response.response?.statusCode == 401 {
